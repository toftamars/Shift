const { Client } = require('pg');
const connectionString = "postgresql://postgres.ttkefeagnivgxejmwhjr:Mars2017Mars@aws-1-eu-central-1.pooler.supabase.com:5432/postgres";
const client = new Client({ connectionString, ssl: { rejectUnauthorized: false } });

async function seedShifts() {
    try {
        await client.connect();

        // Süreyi JS tarafında hesapla
        const calculateDuration = (start, end) => {
            if (start === end) return 0; // OFF
            let s = parseInt(start.split(':')[0]);
            let e = parseInt(end.split(':')[0]);
            if (e < s) e += 24; // Gece yarısı geçişi
            return e - s;
        };

        const shifts = [
            { name: '1', description: 'Sabahçı', start: '09:00', end: '18:00', color: '#FFFFFF' }, // 1: Beyaz (Normal)
            { name: '2', description: 'Kapanışçı', start: '12:00', end: '21:00', color: '#4CAF50' }, // 2: Yeşil
            { name: 'OFF', description: 'İzin', start: '00:00', end: '00:00', color: '#F44336' } // OFF: Kırmızı
        ];

        for (const s of shifts) {
            const duration = calculateDuration(s.start, s.end);

            const check = await client.query('SELECT id FROM shift_types WHERE name = $1', [s.name]);

            if (check.rows.length === 0) {
                await client.query(
                    `INSERT INTO shift_types (name, start_time, end_time, color_code, duration_hours)
             VALUES ($1, $2, $3, $4, $5)`,
                    [s.name, s.start, s.end, s.color, duration]
                );
                console.log(`Vardiya eklendi: ${s.name}`);
            } else {
                await client.query(
                    `UPDATE shift_types SET start_time = $2, end_time = $3, color_code = $4, duration_hours = $5
             WHERE name = $1`,
                    [s.name, s.start, s.end, s.color, duration]
                );
                console.log(`Vardiya güncellendi: ${s.name}`);
            }
        }
    } catch (e) { console.error(e); }
    finally { await client.end(); }
}
seedShifts();
